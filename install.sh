...
# Download Hyper.zip if not found
if [[ ! -f "$SCRIPT_DIR/Hyper.zip" ]]; then
    echo -e "${YELLOW}[INFO] Hyper.zip not found. Downloading...${NC}"
    DOWNLOAD_URL="https://github.com/CodeByCruel/HYPER-KI-MKC/releases/latest/download/Hyper.zip"

    if command -v curl >/dev/null 2>&1; then
        curl -fsSL --retry 3 --retry-delay 2 -o "$SCRIPT_DIR/Hyper.zip" "$DOWNLOAD_URL"
    elif command -v wget >/dev/null 2>&1; then
        wget -q -O "$SCRIPT_DIR/Hyper.zip" "$DOWNLOAD_URL"
    else
    ...
    if [[ ! -f "$SCRIPT_DIR/Hyper.zip" ]]; then
        echo -e "${RED}[ERROR] Failed to download Hyper.zip${NC}"
        echo "Download manually from: $DOWNLOAD_URL"
